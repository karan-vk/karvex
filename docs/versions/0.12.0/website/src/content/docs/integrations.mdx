...
On macOS, Herdr additionally mirrors the shim to `~/.local/bin/tmux` so it wins ordering fights with Homebrew's PATH setup; Herdr only creates that mirror when `~/.local/bin` already exists and never overwrites a real file or a symlink it doesn't own. There is no `herdr uninstall` command, so removing the shim is manual — and the macOS mirror in particular outlives an uninstall of Herdr and keeps shadowing a real `tmux` until it is removed. Confirm each link belongs to Herdr before removing it:

```bash
readlink ~/.config/herdr/shims/tmux   # the shim itself; sessions have their own copy
rm ~/.config/herdr/shims/tmux

readlink ~/.local/bin/tmux            # macOS only; remove only if it points into a Herdr install
rm ~/.local/bin/tmux
```
