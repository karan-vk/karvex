...
複数ノードからなるエージェントワークフローを作成・実行します — タスクのグラフを保存・バージョン管理し、各ノードを実際のターミナルペインとして起動して、すべての分岐が解決するまで調整します。定義フォーマット、ノードの契約、DAG ビューについては[ワークフロー](/docs/workflows/)を参照してください。
...
`show`、`create`、`update`、`run start`、`run show`、`node show` はデフォルトで人間が読めるテキストを出力します。機械可読な JSON エンベロープが必要なときは `--json` を指定してください。`--json` なしの `show` は、ワークフローの概要、バージョン履歴、そしてヘッドバージョンのノード/エッジ/宣言済み引数を表示します。`--json` なしの `run show` は、実行が一時停止しているか、あるノードが対応を必要としているときに、責任のあるノードをその blocker の理由と再開条件とともに表示します。`run start` は、ワークフローで宣言されていない `--arg` キーを黙って無視せず、実行開始前に拒否します。
