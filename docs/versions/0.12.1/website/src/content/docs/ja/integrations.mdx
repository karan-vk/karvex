...
Claude Code の Agent Teams 機能は、`teammateMode` が `auto`(デフォルト)に解決されるとき、各チームメイトを tmux バックエンド経由で起動します。Herdr は自動的にそのバックエンドになりすまします: Herdr が管理するすべてのペインは tmux 互換のセッション識別情報をエクスポートし、Herdr はそのペインの `PATH` に、実際の tmux サーバーではなくローカルの Herdr ソケットと通信する `tmux` という名前の実行ファイルを配置します。Claude Code は `$TMUX` が設定されていることを検出してそのペインを tmux とみなし、各チームメイトを同じタブ内のネイティブな Herdr ペインとして起動します — 別のターミナルマルチプレクサも、アタッチすべきネストしたセッションも不要です。リーダーペインはフォーカスを保持し、チームメイトペインはサイドバーに表示され、他の検出済み Claude Code ペインと同じ方法でライフサイクル状態を報告します。

これには `teammateMode` が実際に `auto` に解決される必要があります。Claude の設定ファイルが `teammateMode` を別の値(例えば `iterm2`)に固定している場合、tmux バックエンドは無効になり、Herdr には Claude 自身の設定の外側からそれを上書きする方法がありません — チームメイトが Herdr ペインとして表示されない場合は、まずこの設定を確認してください。

2 つの環境変数がオプトアウトを提供し、どちらもペインの起動時またはサーバー起動時に読み込まれます:

```bash
KARVEX_NO_TMUX_COMPAT=1   # 新しいペインに tmux 識別情報をエクスポートしない
KARVEX_NO_AUTO_INTEGRATION=1   # サーバー起動時に Claude Code フックを自動インストールしない
```

`KARVEX_NO_TMUX_COMPAT=1` は、Herdr がすでにインストールしたシムを削除しません。設定後に起動するペインへの識別情報のエクスポートを停止するだけです。Herdr ペイン内ですでに `PATH` 上にある実際の `tmux` は通常どおり動作します — Herdr のシムは Claude Code のチームメイトバックエンドが使う狭いコマンド範囲だけを横取りし、無関係なサーバーを指す `tmux -L <name>` の呼び出しを含め、それ以外はすべて素通しします。

`PATH` の最終的な順序を決めるのはペインのシェルです。macOS の `path_helper`、`brew shellenv`、`fish_add_path`、mise、asdf、nix、そして単純な `export PATH="...:$PATH"` はいずれも、Herdr がペインに渡したあとで `PATH` を並べ替え、Herdr のシムディレクトリを後方へ追いやることがよくあります。それが問題になるのは、先に並んだディレクトリのどれかに本物の `tmux` がある場合だけです。そのとき Claude Code はその tmux を Herdr のソケットに対して実行し、呼び出しは失敗し、Claude Code はペインを生成する代わりに黙ってインプロセスのチームメイトにフォールバックします。

そのため、`PATH` 上に別の `tmux` が存在する場合に限り、Herdr は自分自身のバイナリの隣にもシムを設置します。設置先は `herdr` を名前で実行できるようにしている `PATH` 上のディレクトリ、つまりあなたの `PATH` の中ですでに Herdr のものである唯一の場所です。それ以外には手を触れません。Herdr はディレクトリを作成せず、パッケージマネージャーのプレフィックス(Homebrew、Nix、`/usr`)には書き込まず、自分が所有していない実ファイルやシンボリックリンクを置き換えることもありません。さらに、Herdr が `PATH` 上に名前で見つかる形でインストールされていない限り、自分のデータディレクトリの外には一切書き込みません。`PATH` のどこにも他の `tmux` がなければこれは不要なので、何も書き込まれません。

その `tmux` が `PATH` 上で Herdr 自身のディレクトリより前にある場合、あるいは Herdr がパッケージマネージャーのプレフィックス内にインストールされていて書き込みを行わない場合、Herdr は何も設置せず、サーバーログに警告を記録します。その場合、チームメイトは Herdr のペインの外で起動することがあります。`herdr` を含むディレクトリを競合するディレクトリより前に移動すれば解決します。

ミラーは実行するすべてのコマンドから見える `PATH` 上にあるため、意図的に透過的です。Herdr のペインの外や、実際の tmux セッションの中では、シムはすべてのコマンドを本物の `tmux` へそのまま渡し、Herdr 自身のペインから応答することはありません。

`herdr uninstall` コマンドは存在しないため、シムの削除は手動です。特にミラーは Herdr をアンインストールしても残り、削除するまで実際の `tmux` を隠し続けます。削除する前に、それぞれのリンクが Herdr のものであることを確認してください:

```bash
readlink ~/.config/herdr/shims/tmux   # シム本体。セッションごとに別のコピーがある
rm ~/.config/herdr/shims/tmux

readlink ~/.local/bin/tmux            # 必要だった場合に `herdr` の隣へ置かれるもの。Herdr のインストール先を指している場合だけ削除する
rm ~/.local/bin/tmux
```

名前付きセッションは `~/.config/herdr/sessions/<name>/shims/tmux` に独自のコピーを持ちます。`KARVEX_NO_TMUX_COMPAT` も設定していない限り、どちらのリンクも次にペインが起動したときに再作成されるため、削除を恒久的にしたい場合は先にオプトアウトを設定してください。

ネイティブペインとしての Claude Agent Teams は Windows ではサポートされていません。Herdr はそこでは tmux 識別情報をエクスポートしないため、Claude Code はチームメイトに対して独自の tmux 以外のバックエンドにフォールバックします。
