...
单个 `events.subscribe` 连接会按照统一的全局顺序投递所有已订阅的事件:某个节点的 `workflow.node.created` 总是先于该节点自身的 `workflow.node.updated`/`workflow.node.output_checkpoint` 事件到达,而 `workflow.run.finished` 总是晚于该运行的所有节点事件到达。
