...
A single `events.subscribe` connection delivers every subscribed event in one global order: a node's `workflow.node.created` always precedes that same node's own `workflow.node.updated`/`workflow.node.output_checkpoint` events, and `workflow.run.finished` always follows every node event from that run.
