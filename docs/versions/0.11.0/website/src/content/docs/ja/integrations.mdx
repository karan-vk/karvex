...
macOS では、Herdr は Homebrew の PATH 設定との順序の争いに勝つため、シムを `~/.local/bin/tmux` にもミラーします。Herdr はこのミラーを `~/.local/bin` がすでに存在する場合にのみ作成し、自分が所有していない実ファイルやシンボリックリンクを上書きすることはありません。`herdr uninstall` コマンドは存在しないため、シムの削除は手動です。特に macOS のミラーは Herdr をアンインストールしても残り、削除するまで実際の `tmux` を隠し続けます。削除する前に、それぞれのリンクが Herdr のものであることを確認してください:

```bash
readlink ~/.config/herdr/shims/tmux   # シム本体。セッションごとに別のコピーがある
rm ~/.config/herdr/shims/tmux

readlink ~/.local/bin/tmux            # macOS のみ。Herdr のインストール先を指している場合だけ削除する
rm ~/.local/bin/tmux
```
