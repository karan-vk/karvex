...
编写并运行多节点智能体工作流——一个已保存、有版本管理的任务图,将每个节点作为真实的终端窗格启动,并协调它们直到每条分支都得出结果。定义格式、节点约定和 DAG 视图请参见[工作流](/docs/workflows/)。
...
`show`、`create`、`update`、`run start`、`run show` 和 `node show` 默认输出人类可读文本;传入 `--json` 可改为输出机器可读的信封结构。不带 `--json` 的 `show` 会渲染工作流摘要、版本历史,以及头版本的节点/边/声明的参数。不带 `--json` 的 `run show` 会在运行暂停或某个节点需要处理时,指出负责的节点及其 blocker 原因和恢复条件。`run start` 会在运行开始前拒绝工作流未声明的 `--arg` 键,而不是默默丢弃它。
