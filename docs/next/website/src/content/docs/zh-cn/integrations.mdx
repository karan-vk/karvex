...
在 macOS 上,Herdr 还会把垫片镜像到 `~/.local/bin/tmux`,以便在与 Homebrew 的 PATH 设置的顺序之争中获胜;Herdr 只在 `~/.local/bin` 已经存在时才创建这个镜像,并且从不覆盖它不拥有的真实文件或符号链接。目前没有 `herdr uninstall` 命令,因此删除垫片需要手动完成。尤其是 macOS 上的镜像会在卸载 Herdr 之后继续存在,在删除之前一直遮蔽真正的 `tmux`。删除之前先确认每个链接确实属于 Herdr:

```bash
readlink ~/.config/herdr/shims/tmux   # 垫片本体;每个会话另有自己的副本
rm ~/.config/herdr/shims/tmux

readlink ~/.local/bin/tmux            # 仅限 macOS;只有指向 Herdr 安装位置时才删除它
rm ~/.local/bin/tmux
```
