...
1 つの `events.subscribe` 接続は、購読しているすべてのイベントを 1 つのグローバルな順序で配信します。あるノードの `workflow.node.created` は、そのノード自身の `workflow.node.updated`・`workflow.node.output_checkpoint` イベントより必ず先に届き、`workflow.run.finished` はそのランのすべてのノードイベントより必ず後に届きます。
