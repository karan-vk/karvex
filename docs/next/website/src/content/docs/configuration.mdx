...
- `session` — the agent's own name for its current session, so several sessions of the same agent in one workspace stay distinguishable. Falls back to a short session id until the agent names the session, and disappears for panes with no agent session.
