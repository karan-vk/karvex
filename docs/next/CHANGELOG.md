...
- `kvx workflow node expand --label <text>` — a required flag — is no longer discarded either. An expansion child is now named by the label its proposing node gave it, instead of every child cut from one template sharing that template's own label indistinguishably (a fanout of `Worker`, `Worker`, `Worker`, …), which made a node unable to tell its own children apart on the one surface that names them.
