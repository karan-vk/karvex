...
### Added
- Workflow nodes can now grow the graph while a run executes. A node marked `is_template = true` is never scheduled directly; a node that declares `expand_allow = [...]` and `expand_max = N` can ask for instances of those templates with `kvx workflow node expand <run_id> <path> --template <key> --label <text> [--input KEY=VALUE]... [--count N]`, authenticated by the same node token `kvx workflow node complete` uses. A node proposes and Karvex decides: every proposal is judged against the proposing node's `expand_allow` and `expand_max` and the run's `max_depth` and `max_nodes`, and children are created as `<parent>/<template>/<n>`, starting only once their parent has settled.
...
### Changed
- A run's growth ceilings are now enforced, persisted, and reported from one place. `--tier medium` caps a run at 24 nodes and `--tier low` at 12, narrowing the document's `max_nodes` and never raising it, so a run's banner cannot contradict its own record. `max_nodes` counts every node the run has materialised regardless of how it ended — a failed or skipped child does not refund budget — and `max_depth` counts generations of expansion rather than graph layers, so nodes written in the document are all depth 0.
- Each run node's model and effort are now resolved once at run start and written verbatim, together with the reason string behind them, instead of being resolved separately by the live engine and by the store's read path. `kvx workflow node show` explains an `auto` assignment rather than only stating it.
- A workflow notice no longer displaces the one before it. Notices queue and are shown in turn, so a growth refusal raised while another notice is on screen is still seen.
- `kvx workflow show` now reports the head version's `description` and `default_tier` instead of the values the workflow was first created with; `kvx workflow update` writes the authored document's metadata onto the workflow it heads.
...
- A finished run no longer reports two different start times depending on whether it is read from the live engine or from the store. The engine stamps `started_at` once and the database no longer mints a competing value.
