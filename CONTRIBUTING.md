...
cargo feature configurations: the default feature set (`workflow` on, the
build that ships as the single `kvx-<target>` binary) and
`--no-default-features` (the slim, workflow-off build).
