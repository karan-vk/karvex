...
";
    ...
    fn expand_tilde_path_uses_home_when_available() {
        assert_eq!(
            expand_tilde_path_from_env("~/.karvex/worktrees", false, |key| match key {
                "HOME" => Some("/home/me".into()),
                _ => None,
            }),
            PathBuf::from("/home/me/.karvex/worktrees")
        );
        assert_eq!(
    ...
    fn home_dir_uses_windows_profile_before_literal_home() {
        assert_eq!(
            home_dir_from_env(true, |key| match key {
                "HOME" => Some("~".into()),
                "USERPROFILE" => Some(r"C:\Users\karvex".into()),
                _ => None,
            }),
            Ok(PathBuf::from(r"C:\Users\karvex"))
        );
    }
    ...
    fn home_dir_uses_windows_drive_and_path_when_profile_is_missing() {
        assert_eq!(
            home_dir_from_env(true, |key| match key {
                ...
                "HOMEPATH" => Some(r"\Users\karvex".into()),
                _ => None,
            }),
            Ok(PathBuf::from(r"C:\Users\karvex"))
        );
    ...
    fn home_dir_rejects_incomplete_windows_drive_and_path() {
        ...
        assert_eq!(
            home_dir_from_env(true, |key| match key {
                "HOMEDRIVE" => Some("C:".into()),
                "HOMEPATH" => Some("Users\\karvex".into()),
                _ => None,
            }),
            Err(())
        );
    ...
    fn non_windows_tilde_expansion_keeps_windows_separator_literal() {
        assert_eq!(
            expand_tilde_path_from_env(r"~\.karvex\worktrees", false, |key| match key {
                "HOME" => Some("/home/me".into()),
                _ => None,
            }),
            PathBuf::from(r"~\.karvex\worktrees")
        );
    }
    ...
    fn windows_tilde_expansion_normalizes_separators() {
        ...
        let default_path = expand_tilde_path_from_env("~/.karvex/worktrees", true, env);
        assert_eq!(
            default_path,
            PathBuf::from(r"C:\Users\karvex\.karvex\worktrees")
        );
        assert_eq!(
            default_path.display().to_string(),
            r"C:\Users\karvex\.karvex\worktrees"
        );
        assert_eq!(
            expand_tilde_path_from_env(r"~\.karvex\worktrees", true, env),
            PathBuf::from(r"C:\Users\karvex\.karvex\worktrees")
        );
    }
    ...
    fn default_checkout_path_appends_repo_and_branch_slug() {
        assert_eq!(
            default_checkout_path(
                Path::new("/home/me/.karvex/worktrees"),
                "karvex",
                "worktree/brave-river",
            ),
            PathBuf::from("/home/me/.karvex/worktrees/karvex/worktree-brave-river")
        );
    }
    ...
    fn worktree_remove_command_preserves_branch_by_not_deleting_it() {
        let command = build_worktree_remove_command(
            Path::new("/repo/karvex"),
            Path::new("/w/karvex/issue-137"),
            false,
        );
    ...
    fn dirty_remove_error_detection_matches_git_force_hint() {
        assert!(is_dirty_worktree_remove_error(
            "fatal: '/w/karvex' contains modified or untracked files, use --force to delete it"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/karvex' is a missing but already registered worktree"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/karvex' contains a locked worktree, use --force only if you know why"
        ));
    }
    ...
    fn worktree_add_command_creates_new_branch_from_base() {
        let command = build_worktree_add_new_branch_command(
            Path::new("/repo/karvex"),
            Path::new("/w/karvex/worktree-brave-river"),
            "worktree/brave-river",
            "HEAD",
        );
        assert_eq!(command.program, "git");
