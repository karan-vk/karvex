...
/// Serializable snapshot of the entire karvex session.
...
mod tests {
    ...
    fn session_fixture(name: &str) -> &'static str {
        match name {
            "current-karvex" => {
                include_str!("../../tests/fixtures/session/current-karvex-session.json")
            }
            "current-karvex-dev" => {
                include_str!("../../tests/fixtures/session/current-karvex-dev-session.json")
            }
            "legacy-pre-tabs-v2" => {
    ...
    fn round_trip_full_workspace_snapshot() {
        ...
        panes.insert(
            0,
            PaneSnapshot {
                cwd: PathBuf::from("/home/can/Projects/karvex"),
                label: None,
                agent_name: None,
        ...
        let snap = SessionSnapshot {
            workspaces: vec![WorkspaceSnapshot {
                ...
                identity_cwd: PathBuf::from("/home/can/Projects/karvex"),
                worktree_space: None,
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/can/Projects/karvex")
        );
        assert_eq!(
    ...
    fn current_session_fixture_parses() {
        let snap = parse_snapshot(session_fixture("current-karvex")).unwrap();
    ...
    fn current_dev_session_fixture_parses_additive_fields() {
        let snap = parse_snapshot(session_fixture("current-karvex-dev")).unwrap();
    ...
    fn legacy_workspace_snapshot_migrates_to_single_tab() {
        ...
        assert_eq!(ws.tabs[0].panes[&1].cwd, PathBuf::from("/tmp/karvex"));
    }
    ...
    fn capture_contract_tracks_worktree_space_membership() {
        ...
        state.workspaces[0].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            key: "repo-key".into(),
            label: "karvex".into(),
            repo_root: PathBuf::from("/repo/karvex"),
            checkout_path: PathBuf::from("/repo/karvex/worktree-a"),
    ...
    fn capture_contract_tracks_workspace_identity_and_pane_cwds() {
        ...
        let second_terminal_id = state.workspaces[0].tabs[0].panes[&second]
            .attached_terminal_id
            .clone();
        state.terminals.get_mut(&second_terminal_id).unwrap().cwd = PathBuf::from("/tmp/karvex");
        ...
        assert_eq!(tab.panes[&second.raw()].cwd, PathBuf::from("/tmp/karvex"));
    }
    ...
    fn capture_contract_tracks_hook_authority_agent_session() {
        ...
        assert_eq!(agent_session.source, "karvex:pi");
    ...
    fn capture_contract_preserves_restored_agent_session() {
        ...
        assert_eq!(agent_session.source, "karvex:opencode");
    ...
    fn restore_falls_back_to_home_when_cwd_missing() {
        ...
        panes.insert(
            0,
            PaneSnapshot {
                cwd: PathBuf::from("/tmp/this-directory-does-not-exist-for-karvex-test"),
                label: None,
                agent_name: None,
                managed_agent_kind: None,
                agent_session: None,
        ...
        assert_eq!(
            restored.workspaces[0].tabs[0].panes[&0].cwd,
            PathBuf::from("/tmp/this-directory-does-not-exist-for-karvex-test")
        );
    }
}
