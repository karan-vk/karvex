...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: kvx api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Karvex API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `kvx api schema --json` to print the full schema.\nUse `kvx api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("kvx api commands:");
    eprintln!("  kvx api snapshot");
    eprintln!("  kvx api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: kvx api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Karvex API schema"));
        assert!(text.contains("Use `kvx api schema --json`"));
        assert!(text.len() < 400);
    }
}
