...
/// Reusable client for Karvex's newline-delimited JSON API.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn local_session_target_resolves_named_session_socket() {
        ...
        assert!(client.socket_path().ends_with("sessions/work/karvex.sock"));
    }

    #[test]
    fn socket_path_target_uses_explicit_path() {
        let path = PathBuf::from("/tmp/karvex-test.sock");
