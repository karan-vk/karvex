...
fn prepare_socket_path(path: &Path) -> std::io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "karvex is already running (socket busy at {})",
            path.display()
        )
    })
}
...
fn api_method_name(method: &Method) -> &'static str {
    match method {
        ...
        Method::WorkflowRunReportSession(_) => "workflow.run.report_session",
        Method::WorkflowRunMessage(_) => "workflow.run.message",
...
mod tests {
    ...
    fn socket_path_prefers_explicit_env_override() {
        ...
        let unique = format!("/tmp/karvex-test-{}.sock", std::process::id());
