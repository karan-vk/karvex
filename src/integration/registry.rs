...
pub(crate) fn print_outdated_update_notice() -> bool {
    ...
    let targets = outdated
        ...
        .collect::<Vec<_>>();
    eprintln!(
        "installed karvex integrations need updating; {}.",
        integration_update_instructions(&targets).replace('`', "")
    );
    true
}

/// Whether the Karvex-owned Grok hook config exactly matches the installed
...
pub(crate) fn integration_status_at(
    ...
    // Grok only invokes the hook when the karvex-owned `hooks/karvex.json`
    // registers it, so a current hook script with a missing or broken config
    // is a nonfunctional install: report it as outdated so `kvx integration
