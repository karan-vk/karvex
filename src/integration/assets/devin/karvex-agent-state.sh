...
# KARVEX_INTEGRATION_ID=devin
# KARVEX_INTEGRATION_VERSION=2

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/karvex-devin-hook.XXXXXX")" || exit 0
...
KARVEX_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
...
def load_session_list(project_dir: str | None):
    injected = os.environ.get("KARVEX_DEVIN_LIST_JSON")
...
hook_input = load_hook_input(os.environ.get("KARVEX_HOOK_INPUT_FILE"))
