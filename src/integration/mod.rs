...
pub(crate) use env::{
    apply_pane_base_env, claude_dir, KARVEX_PANE_ID_ENV_VAR, KARVEX_TAB_ID_ENV_VAR,
    KARVEX_WORKSPACE_ID_ENV_VAR,
};
...
const PI_EXTENSION_INSTALL_NAME: &str = "karvex-agent-state.ts";
const PI_EXTENSION_ASSET: &str = include_str!("assets/pi/karvex-agent-state.ts");
const PI_INTEGRATION_VERSION: u32 = 8;
const OMP_EXTENSION_INSTALL_NAME: &str = "karvex-omp-agent-state.ts";
const OMP_EXTENSION_ASSET: &str = include_str!("assets/omp/karvex-agent-state.ts");
const OMP_INTEGRATION_VERSION: u32 = 8;
const CLAUDE_HOOK_INSTALL_NAME: &str = if cfg!(windows) {
    "karvex-agent-state.ps1"
} else {
    "karvex-agent-state.sh"
};
const CLAUDE_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/claude/karvex-agent-state.ps1")
} else {
...
const CODEX_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/codex/karvex-agent-state.ps1")
} else {
    include_str!("assets/codex/karvex-agent-state.sh")
};
...
const KIMI_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/kimi/karvex-agent-state.ps1")
} else {
    include_str!("assets/kimi/karvex-agent-state.sh")
};
const KIMI_INTEGRATION_VERSION: u32 = 6;
const KIMI_CONFIG_BLOCK_BEGIN: &str = "# >>> karvex kimi integration";
const KIMI_CONFIG_BLOCK_END: &str = "# <<< karvex kimi integration";
...
const COPILOT_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/copilot/karvex-agent-state.ps1")
} else {
    include_str!("assets/copilot/karvex-agent-state.sh")
};
...
const DEVIN_HOOK_INSTALL_NAME: &str = "karvex-agent-state.sh";
const DEVIN_HOOK_ASSET: &str = include_str!("assets/devin/karvex-agent-state.sh");
...
const DROID_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/droid/karvex-agent-state.ps1")
} else {
    include_str!("assets/droid/karvex-agent-state.sh")
};
...
const OPENCODE_PLUGIN_INSTALL_NAME: &str = "karvex-agent-state.js";
const OPENCODE_PLUGIN_ASSET: &str = include_str!("assets/opencode/karvex-agent-state.js");
const OPENCODE_INTEGRATION_VERSION: u32 = 9;
const KILO_PLUGIN_INSTALL_NAME: &str = "karvex-agent-state.js";
const KILO_PLUGIN_ASSET: &str = include_str!("assets/kilo/karvex-agent-state.js");
const KILO_INTEGRATION_VERSION: u32 = 4;
const HERMES_PLUGIN_INSTALL_NAME: &str = "karvex-agent-state";
...
const QODERCLI_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/qodercli/karvex-agent-state.ps1")
} else {
    include_str!("assets/qodercli/karvex-agent-state.sh")
};
...
const CURSOR_HOOK_INSTALL_NAME: &str = "karvex-agent-state.sh";
const CURSOR_HOOK_ASSET: &str = include_str!("assets/cursor/karvex-agent-state.sh");
const CURSOR_INTEGRATION_VERSION: u32 = 1;
#[cfg(windows)]
const ANTIGRAVITY_CLI_HOOK_INSTALL_NAME: &str = "karvex-agent-state.ps1";
#[cfg(not(windows))]
const ANTIGRAVITY_CLI_HOOK_INSTALL_NAME: &str = "karvex-agent-state.sh";
#[cfg(windows)]
const ANTIGRAVITY_CLI_HOOK_ASSET: &str =
    include_str!("assets/antigravity_cli/karvex-agent-state.ps1");
#[cfg(not(windows))]
const ANTIGRAVITY_CLI_HOOK_ASSET: &str =
    include_str!("assets/antigravity_cli/karvex-agent-state.sh");
const ANTIGRAVITY_CLI_INTEGRATION_VERSION: u32 = 1;
/// Antigravity CLI keys `hooks.json` by hook name, so every Karvex entry lives
/// under one Karvex-owned block that install rewrites and uninstall removes.
const ANTIGRAVITY_CLI_HOOK_BLOCK_NAME: &str = "karvex";
...
const INTEGRATION_VERSION_MARKER: &str = "KARVEX_INTEGRATION_VERSION=";
const MASTRACODE_HOOK_INSTALL_NAME: &str = "karvex-agent-state.sh";
const MASTRACODE_HOOK_ASSET: &str = include_str!("assets/mastracode/karvex-agent-state.sh");
...
const GROK_HOOK_INSTALL_NAME: &str = "karvex-agent-state.sh";
const GROK_HOOK_CONFIG_INSTALL_NAME: &str = "karvex.json";
const GROK_HOOK_ASSET: &str = include_str!("assets/grok/karvex-agent-state.sh");
