...
struct RawPluginManifest {
    ...
    version: String,
    #[serde(default)]
    min_karvex_version: Option<String>,
    #[serde(default)]
...
pub(crate) fn load_plugin_manifest(
    ...
    let min_karvex_version = validate_min_karvex_version(raw.min_karvex_version.as_deref())?;
    let description = raw
        .description
...
fn validate_min_karvex_version_against(
    ...
    let Some(value) = value else {
        return Err((
            ...
            "plugin min_karvex_version is required".to_string(),
        ));
    };
    let value = non_empty_trimmed(
        value,
        "invalid_plugin_min_karvex_version",
        "plugin min_karvex_version is required",
    )?;
    let required = crate::update::Version::parse(&value).ok_or_else(|| {
        (
            "invalid_plugin_min_karvex_version",
            format!(
                "plugin min_karvex_version must be a semantic version like {}",
    ...
    if required > current {
        return Err((
            ...
            format!("plugin requires Karvex {required} or newer; current Karvex is {current}"),
        ));
    }
