...
pub fn config_diagnostic_summary(diagnostics: &[String]) -> Option<String> {
    ...
    Some(format!("{target}{impact}; kvx config check"))
}
...
mod tests {
    ...
    fn remove_section_key_removes_matching_key_from_section() {
        ...
        assert!(!updated.contains("[ui.toast]\nenabled = true"));
        assert!(updated.contains("delivery = \"karvex\""));
    ...
    fn config_diagnostic_summary_reports_default_fallback() {
        ...
        assert_eq!(
            config_diagnostic_summary(&diagnostics).as_deref(),
            Some("config.toml invalid; using defaults; kvx config check")
        );
    }
    ...
    fn config_diagnostic_summary_reports_unreadable_config_impact() {
        ...
        assert_eq!(
            ...
            Some("config.toml unreadable; using defaults; kvx config check")
        );
        ...
        assert_eq!(
            ...
            Some("config.toml unreadable; keeping current config; kvx config check")
        );
    }
    ...
    fn config_diagnostic_summary_reports_retained_live_config() {
        ...
        assert_eq!(
            config_diagnostic_summary(&diagnostics).as_deref(),
            Some("config.toml invalid; keeping current config; kvx config check")
        );
    }
    ...
    fn config_loaders_report_unreadable_path() {
        ...
        let path =
            std::env::temp_dir().join(format!("karvex-config-unreadable-{}", std::process::id()));
...
"#,
        ...
        assert_eq!(
            ...
            super::super::ToastDelivery::Karvex
        );
    }
...
"#,
    ...
    fn startup_config_accepts_legacy_agent_panel_scope_without_warning() {
        ...
        let path = std::env::temp_dir().join(format!(
            "karvex-config-legacy-agent-panel-scope-{}.toml",
            std::process::id()
        ));
    ...
    fn startup_config_load_warns_about_unknown_top_level_sections() {
        let _guard = crate::config::test_config_env_lock().lock().unwrap();
        let path = std::env::temp_dir().join(format!(
            "karvex-config-unknown-section-{}.toml",
            std::process::id()
        ));
        std::fs::write(
            &path,
