...
pub(crate) fn init_file_logging(file_name: &str) {
    ...
    let filter =
        EnvFilter::try_from_env("KARVEX_LOG").unwrap_or_else(|_| EnvFilter::new("kvx=info"));
...
pub(crate) fn help_log_paths_summary() -> String {
    ...
    format!(
        "{} (plus karvex-client.log, karvex-server.log)",
        dir.join("karvex.log").display()
    )
}
...
mod tests {
    ...
    fn temp_log_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(unique).join("karvex.log")
    }

    #[test]
    fn rotated_log_path_appends_numeric_suffix() {
        let path = PathBuf::from("/tmp/karvex.log");
        assert_eq!(
            rotated_log_path(&path, 2),
            PathBuf::from("/tmp/karvex.log.2")
        );
    }
    ...
    fn write_replaces_log_without_retained_files_when_size_limit_is_reached() {
        ...
        let writer = RotatingFileMakeWriter::new(dir.clone(), "karvex.log", 8, 0).unwrap();
        {
