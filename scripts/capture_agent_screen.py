#!/usr/bin/env python3
"""Capture repeated Karvex pane reads for agent screen detection fixtures."""
...
def main() -> int:
    ...
    capture_index = 1
    while True:
        pane = resolve_target(args.kvx, args.pane, args.agent)
        if pane is None:
            if args.agent:
                print(f"agent '{args.agent}' was not found by `kvx agent get`")
            else:
                print(f"pane '{args.pane}' was not found by `kvx pane list`")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Interactively capture Karvex pane screen reads for agent detection fixture work."
    )
    parser.add_argument(
        "--pane",
    ...
    parser.add_argument(
        "--agent",
        help="agent target to capture; resolved with `kvx agent get`",
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    parser.add_argument(
        "--kvx",
        default="kvx",
        help="Karvex CLI binary to call (default: kvx)",
    )
    parser.add_argument(
        "--once",
        action="store_true",
...
def resolve_pane(karvex: str, pane_ref: str) -> PaneMatch | None:
    result = run_command([karvex, "pane", "list"])
    if result.code != 0:
        pane = fallback_pane_id(pane_ref)
        return enrich_pane_with_agent(karvex, pane) if pane else None
    ...
    if len(matches) == 1:
        return enrich_pane_with_agent(karvex, pane_from_dict(matches[0]))
    if len(matches) > 1:
...
def resolve_target(karvex: str, pane_ref: str, agent_ref: str | None) -> PaneMatch | None:
    if agent_ref:
        agent = get_agent(karvex, agent_ref)
        if agent is None:
            return None
        pane = pane_from_agent_dict(agent)
        return enrich_pane_with_pane_list(karvex, pane)
    return resolve_pane(karvex, pane_ref)


def get_agent(karvex: str, agent_ref: str) -> dict[str, Any] | None:
    result = run_command([karvex, "agent", "get", agent_ref])
    if result.code != 0:
        return None
    try:
        response = json.loads(result.stdout.decode("utf-8"))
...
def enrich_pane_with_agent(karvex: str, pane: PaneMatch) -> PaneMatch:
    result = run_command([karvex, "agent", "list"])
    if result.code != 0:
        return pane
    try:
        response = json.loads(result.stdout.decode("utf-8"))
...
def enrich_pane_with_pane_list(karvex: str, pane: PaneMatch) -> PaneMatch:
    result = run_command([karvex, "pane", "list"])
    if result.code != 0:
        return pane
    try:
    ...
    for index in range(1, args.samples + 1):
        ...
        commands = [
            (
                ...
                [args.kvx, "pane", "read", pane.pane_id, "--source", "detection", "--format", "text"],
            ),
            (
                f"{prefix}.detection.ansi",
                [args.kvx, "pane", "read", pane.pane_id, "--source", "detection", "--format", "ansi"],
            ),
            (
                f"{prefix}.recent.txt",
                [
            ...
            (
                ...
                [args.kvx, "agent", "explain", pane.pane_id, "--json"],
            ),
        ]
...
def write_metadata(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "[commands]",
        'detection_text = "kvx pane read <pane> --source detection --format text"',
        'detection_ansi = "kvx pane read <pane> --source detection --format ansi"',
        'recent_text = "kvx pane read <pane> --source recent --lines <n> --format text"',
        'recent_ansi = "kvx pane read <pane> --source recent --lines <n> --format ansi"',
        'explain = "kvx agent explain <pane> --json"',
    ]
    if failures:
        lines.append("")
