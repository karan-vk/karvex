...
KVX_BIN="${KVX_BIN:-$ROOT/target/debug/kvx}"
BASE="${BASE:-$(mktemp -d /tmp/karvex-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_kvx "$session" server stop >/dev/null 2>&1 || true
  done
...
run_kvx() {
  ...
  env -u KARVEX_SOCKET_PATH \
    -u KARVEX_CLIENT_SOCKET_PATH \
    -u KARVEX_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$KVX_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/karvex-dev' "$CONFIG_HOME"
  else
    printf '%s/karvex-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/karvex.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/karvex-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/karvex-dev/karvex.sock | "$CONFIG_HOME"/karvex-dev/sessions/*/karvex.sock)
      ;;
    *)
...
echo "using kvx: $KVX_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/karvex-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/karvex-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_kvx "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
