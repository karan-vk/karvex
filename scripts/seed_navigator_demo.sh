...
Seeds a running karvex server with navigator demo workspaces, tabs, panes,
and fake agent states for recording the session navigator.

Environment:
  KARVEX_NAV_SOCKET_PATH  API socket to target. Defaults to $HOME/.config/karvex-dev/karvex.sock.
  KARVEX_NAV_CWD          Workspace cwd for created panes. Defaults to the repo root.
  KARVEX_NAV_BIN          Karvex binary to call. Defaults to cargo run from the repo.
USAGE
}
...
workspace_cwd="${KARVEX_NAV_CWD:-$repo_dir}"
config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
dev_socket="$config_home/karvex-dev/karvex.sock"
main_socket="$config_home/karvex/karvex.sock"
export KARVEX_SOCKET_PATH="${KARVEX_NAV_SOCKET_PATH:-$dev_socket}"

if [[ "$allow_main" != 1 && "$KARVEX_SOCKET_PATH" == "$main_socket" ]]; then
  echo "refusing to seed main karvex session: $KARVEX_SOCKET_PATH" >&2
  echo "use KARVEX_NAV_SOCKET_PATH for a dev socket, or pass --allow-main intentionally" >&2
  exit 1
fi

if [[ ! -S "$KARVEX_SOCKET_PATH" ]]; then
  echo "karvex socket not found: $KARVEX_SOCKET_PATH" >&2
  echo "start a dev server first, or set KARVEX_NAV_SOCKET_PATH" >&2
  exit 1
fi

cd "$repo_dir"

if [[ -n "${KARVEX_NAV_BIN:-}" ]]; then
  run() { "$KARVEX_NAV_BIN" "$@"; }
else
  run() { cargo run --quiet -- "$@"; }
fi
...
cat <<EOF
Seeded navigator demo data via $KARVEX_SOCKET_PATH
