...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/karvex-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
/// Spawn `kvx` (no subcommand) — the auto-detect launch path.
fn spawn_karvex_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
    _client_socket_path: &Path,
...
/// Spawn `kvx --no-session` — the monolithic escape hatch.
fn spawn_karvex_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedKarvex {
...
fn run_cli(socket_path: &Path, args: &[&str]) -> std::process::Output {
    ...
    command.args(args);
    command.env("KARVEX_SOCKET_PATH", socket_path);
...
/// Running `kvx` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    // Run `kvx` (no subcommand) — should auto-detect, spawn server, attach as client.
    let karvex = spawn_karvex_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = karvex.child.process_id().expect("client should have PID");
    assert!(
...
/// Running `kvx` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `kvx` (no subcommand) — should detect the running server and attach.
    let client = spawn_karvex_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_karvex(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_karvex(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `KARVEX_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `kvx` with custom socket paths.
    let karvex = spawn_karvex_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `kvx --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `kvx --no-session` — monolithic mode, no server/client.
    let karvex = spawn_karvex_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one karvex process.
    let client_pid = karvex.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `kvx workspace list` through the server's API socket.
    ...
    // Test `kvx pane list` through the server's API socket.
...
/// persists after the client exits, and a new `kvx` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `kvx` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_karvex_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `kvx` again — should detect the running server and reattach.
    let client2 = spawn_karvex_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_karvex(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set KARVEX_SOCKET_PATH or KARVEX_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("karvex.sock");
    let client_socket = config_home.join(app_dir_name).join("karvex-client.sock");
    ...
    cmd.env_remove("KARVEX_SOCKET_PATH");
    cmd.env_remove("KARVEX_CLIENT_SOCKET_PATH");

    let child = pair.slave.spawn_command(cmd).unwrap();
    register_spawned_karvex_pid(child.process_id());
    drop(pair.slave);
    let server = SpawnedKarvex {
        _master: pair.master,
        child,
    };
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_karvex_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    assert!(
        ...
        "persistent client logs should not land in karvex.log: {monolith_content}"
    );
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    assert!(
        stderr.contains("nested karvex is disabled by default"),
