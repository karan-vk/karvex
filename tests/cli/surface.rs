...
fn pane_run_sends_one_send_input_request_with_enter_key() {
    ...
    let socket_path = base.join("karvex.sock");
...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            ...
            "kvx {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
...
fn subcommand_help_explains_automation_semantics_without_a_server() {
    ...
    for (args, expected) in cases {
        ...
        assert!(
            ...
            "kvx {} help did not contain {expected:?}: {stdout}",
            args.join(" ")
        );
    }
}
...
fn removed_wait_and_agent_send_commands_are_rejected() {
    let wait = Command::new(env!("CARGO_BIN_EXE_kvx"))
    ...
    let help = Command::new(env!("CARGO_BIN_EXE_kvx"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(!String::from_utf8_lossy(&help.stdout).contains("kvx wait <subcommand>"));

    let send = Command::new(env!("CARGO_BIN_EXE_kvx"))
        .args(["agent", "send", "reviewer", "hello"])
        .output()
        .unwrap();
    assert_eq!(send.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&send.stderr);
    assert!(stderr.contains("kvx agent send-keys"));
    assert!(!stderr.contains("kvx agent send <"));
}
...
fn agent_cli_rejects_invalid_wait_and_rename_grammar_locally() {
    for args in [
        ...
        let output = Command::new(env!("CARGO_BIN_EXE_kvx"))
            .args(args)
            .env("KARVEX_SOCKET_PATH", "/nonexistent/karvex.sock")
            .output()
            .unwrap();
        assert_eq!(
            output.status.code(),
...
fn completion_command_prints_zsh_script_without_session_startup() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("#compdef kvx"), "stdout: {stdout}");
    assert!(
        !stdout.contains("--cwd=[]"),
...
fn root_help_advertises_api_schema_command_group() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("kvx api <subcommand>"),
...
fn api_schema_default_output_is_a_short_summary() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Karvex API schema"), "stdout: {stdout}");
    assert!(
        stdout.contains("Use `kvx api schema --json`"),
        "stdout: {stdout}"
    );
    assert!(
        stdout.len() < 400,
...
fn api_schema_output_writes_bundled_schema_to_file() {
    let base = unique_test_dir();
    fs::create_dir_all(&base).unwrap();
    let schema_path = base.join("karvex-api.schema.json");
