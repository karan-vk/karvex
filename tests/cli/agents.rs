...
fn agent_start_command_works() {
    ...
    fs::write(
        &fake_pi,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\nexport KARVEX_AGENT=pi\n'{}' pane report-agent \"$KARVEX_PANE_ID\" --source custom:fake-pi --agent pi --state idle >/dev/null\nwhile IFS= read -r prompt; do\n  case \"$prompt\" in \"do not transition\"|\"stall\") continue ;; esac\n  '{}' pane report-agent \"$KARVEX_PANE_ID\" --source custom:fake-pi --agent pi --state working >/dev/null\n  '{}' pane report-agent \"$KARVEX_PANE_ID\" --source custom:fake-pi --agent pi --state idle >/dev/null\n  printf '%s\\n' \"$prompt\" >> '{}'\ndone\n",
            captured_args.display(),
            env!("CARGO_BIN_EXE_kvx"),
            env!("CARGO_BIN_EXE_kvx"),
            env!("CARGO_BIN_EXE_kvx"),
    ...
    let karvex = spawn_karvex_with_path(&config_home, &runtime_dir, &socket_path, Some(&bin));
...
fn agent_start_reports_detected_kind_mismatch_before_released_name() {
    ...
    fs::write(
        &fake_pi,
        "#!/bin/sh\nKARVEX_AGENT=codex exec /bin/sleep 10\n",
    )
    .unwrap();
    fs::set_permissions(&fake_pi, fs::Permissions::from_mode(0o755)).unwrap();
...
fn agent_start_follows_its_named_terminal_when_the_pane_moves() {
    ...
    fs::create_dir_all(&bin).unwrap();
    let fake_pi = bin.join("pi");
    fs::write(&fake_pi, "#!/bin/sh\nKARVEX_AGENT=pi exec /bin/sleep 10\n").unwrap();
    fs::set_permissions(&fake_pi, fs::Permissions::from_mode(0o755)).unwrap();
